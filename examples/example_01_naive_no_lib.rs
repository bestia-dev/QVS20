...
// region: lmake_readme include "DEVELOPMENT.md" //! A

// endregion: lmake_readme include "DEVELOPMENT.md" //! A
...
fn main() {
    ...
    read_with_find("cou_den_rows.qvs20");
    read_with_find("cou_den.qvs20");

    read_with_regex("cou_den_rows.qvs20");
    read_with_regex("cou_den.qvs20");
...
fn write_separate_files(vec_of_rows: &Vec<Qvs20Row>) {
    ...
    let schema_text = "[cou_den][example with country population density]
[String][Decimal]
[][]
[][]
[Country][Density]
";
    unwrap!(fs::write(
        "sample_data/write/cou_den_schema.qvs20",
        &schema_text
    ));
    println!("cou_den_schema.qvs20:");
    println!("{}", schema_text);
    ...
    unwrap!(fs::write(
        "sample_data/write/cou_den_rows.qvs20",
        &rows_text
    ));
    println!("cou_den_rows.qvs20:");
    println!("{}", rows_text);
}
...
fn write_one_file(vec_of_rows: &Vec<Qvs20Row>) {
    ...
    text.push_str(
        "[cou_den][example with country population density]
[String][Decimal]
[][]
[][]
[Country][Density]
",
    ...
    unwrap!(fs::write("sample_data/write/cou_den.qvs20", &text));
    println!("cou_den.qvs20:");
    println!("{}", text);
}
